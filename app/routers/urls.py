...
from app.service.crud import db_create_url, db_get_url_by_key, db_deactivate_url, db_reactivate_url, db_get_all_url

router = APIRouter(prefix="/0",
                   tags=['URLInfo'])


@router.get("/", status_code=status.HTTP_401_UNAUTHORIZED, response_class=HTMLResponse,
            summary='Ping this to see if its hosted')
def read_root():
    html = """<!DOCTYPE html>
<html>
<body>
...
async def peek(url_key, request: Request, session: AsyncSession = Depends(get_session)):
    ...
    return URLBase(target_url=result.target_url, created_on=result.created_on, is_active=result.is_active,
                   key=result.key)
...
async def delete_url_key_binding(url_key, session: AsyncSession = Depends(get_session)):
    ...
    if result == 'Already Deactivated':
        resp = {"status": {"key": "{}".format(url_key), "msg": "Already Deactivated!"}}
        return resp
    else:
        resp = {"status": {"key": "{}".format(url_key), "msg": "Deactivated successfully!"}}
        return resp
